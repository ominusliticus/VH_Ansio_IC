...
# global constant
cdef double hbarc = 0.197326938 

# this class will house the details of the model we want to solve 
# with th Hydro_Sim class above
cdef class Hydro_Model:
	'''
	Class: contains the hydrodynamic equations, specifically the source terms
	for the Kurganov-Tadmor algorithm. These source terms depend on the paradigm
	MIS, 3rd-order, Grad, etc
	'''
	# class member variables
	cdef int    m_model_flag
	cdef double m_eta_s
    def __cinit__(self, model_flag, eta_s):
        '''
    ...
    # ------------------------------------------------------------------
    cdef class transport_coefficients:
		# member variables
		cdef double m_T
		cdef double m_e
		cdef double p_in 
        ...
        def __cinit__(self, model_flag, T_in, e_in, p_in):
            '''
        ...
        cdef void compute_transport_coefficients(self, double eta_s):
            '''
            ...
            model_flag == 2 - MIS linear fluctuation
            '''
            if self.m_model_flag is 1: # MIS full
                # need to edit when ready
                self.tau_pi  = eta_s / self.m_T
                self.beta_pi = (self.m_p + self.m_e) / 5
                return tau_pi, beta_pi
            elif m_model_flag is 2: # MIS linear fluctuation
                # this will come later  
                print('Option not yet available')
            else:
                print('Invalid option')
    
    cdef class spatial_projection:
		cdef double Dtt, Dtx, Dty, Dtn, Dxx, Dxy, Dxn, Dyy, Dyn, Dnn
        def __cinit__(self, 
			double Dtt, 
			double Dtx, 
			double Dty, 
    ...
    cdef void project_spatial(self, 
			double Delta, 
			double At, 
			dounle Ax, 
			double Ay, 
