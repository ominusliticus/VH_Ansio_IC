...
class Hydro_Sim:
	...
	# ------------------------------------------------------------------
	def set_time_step(self, n, tau, dtau_prev, hit_CFL_bound, model):
		'''
			...
			for k in range(2, Neta + 2):
				for j in range(2, Ny + 2):
					for i in range(2, Nx + 2):
						...
						if ax is 0 or ay is 0 or an is 0:
							sys.exit(f'Avoided division by zer0: ax={ax}, ay={ay}, an={an}')
						# take the smallest one
						dt_CFL = np.fmin(dt_CFL, np.fmin(dx / ax, np.fmin(dy / ay, deta / an)))
			return dt_CFL / 8
			
		def compute_dt_source(tau, dt_prev):
			'''
		...
		if (self.m_lattice.adaptive_time_step):
			if n is 0:
				...
				if not hit_CFL_bound:
					update = 0
					self.calculate_euler_step(tau, dtau_prev, dtau_prev, update, hit_CFL_bound, model)
					
					dt_source = compute_dt_source(tau, dtau_prev)
					if dt_source > dt_CFL:
